...
int athena_guildmember = 16;
int athena_guildextension = 4;
...
int mmo_char_tobuf(uint8* buffer, struct mmo_charstatus* p)
{
	...
	WBUFW(buf,56) = p->option&(0x20|0x80000|0x100000|0x200000|0x400000|0x800000|0x1000000|0x2000000|0x4000000) ? 0 : p->weapon;
			...
			{// TODO: Is this even possible?
				...
				for( i = 0; i < num; ++i )
				{
					if( class_[i] == JOB_BARD || class_[i] == JOB_DANCER ||
						...
						class_[i] == JOB_BABY_MINSTREL || class_[i] == JOB_BABY_WANDERER || class_[i] == JOB_KAGEROU || class_[i] == JOB_OBORO )
					{
						// job modification
						if( class_[i] == JOB_BARD || class_[i] == JOB_DANCER )
...
int char_config_read(const char* cfgName)
{
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		} else if (strcmpi(w1, "athena_guildmember") == 0) {
		athena_guildmember = atoi(w2);
		} else if (strcmpi(w1, "athena_guildextension") == 0) {
		athena_guildextension = atoi(w2); 
