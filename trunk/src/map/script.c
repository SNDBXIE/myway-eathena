...
struct Script_Config script_config = {
	...
	"OnPetEvolveEvent",  // pet evo event
...
BUILDIN_FUNC(getelement)
{
	...
	{
		if (!battle_config.left_cardfix_to_right && type == 2)
			script_pushint(st, sd->battle_status.lhw.ele);
		else
			script_pushint(st, sd->battle_status.rhw.ele);
	}
...
BUILDIN_FUNC(scbonus)
{
	...
	if (type < SC_COMMON_MIN || type >= SC_MAX)
	{
		ShowError("scbonus: Invalid status given (%d)\n", type);
		return 0;
	}

	if (sd->sc_effect[type].bonus)
	{
		ShowError("scbonus: A bonus already defined for status %d\n", type);
		return 0;
	}

	sd->sc_effect[type].bonus = parse_script(script_getstr(st, 3), "scbonus", 0, 0);
...
BUILDIN_FUNC(getdropbbbonus){
	...
	if (sd){
		script_pushint(st,((int)(sd->dropbb_bonus*100)));
	}else{
		// Failed to attach player
		script_pushint(st,-1);
	}
...
/*===========================================
 * getitemguild(<item_id>,<amount>{,<guild_id>,"<map_name>"});
 * by: CarlosHenrq
 *-------------------------------------------*/
BUILDIN_FUNC(getitemguild){
	TBL_PC * sd = script_rid2sd(st), *p_sd = NULL;
	struct guild * g;
	struct item item_tmp;
	struct item_data *item_data;
	int nameid,amount,guild,flag,count,i,x,m;
 
	guild = (( script_hasdata(st,4) ) ? script_getnum(st,4):sd->status.guild_id);
	
	if( (g = guild_search(guild)) != NULL ){
		if( (item_data = itemdb_exists(script_getnum(st,2))) != NULL ){
			nameid = item_data->nameid;
			count = amount = script_getnum(st,3);
			if( !itemdb_isstackable2(item_data) )
				amount = 1;
			for( i = 0; i < MAX_GUILD; i++ ){
				if( (p_sd = map_id2sd(g->member[i].account_id)) != NULL ){
					for( x = 0; x < count; x += amount ){
						if( !pet_create_egg(p_sd,nameid) ){
							memset(&item_tmp,0,sizeof(item_tmp));
							item_tmp.nameid = nameid;
							item_tmp.identify = 1;
							if( script_hasdata(st,5) ){
								if( (m = map_mapname2mapid(script_getstr(st,5))) >= 0 )
									if(p_sd->bl.m = m){
										if( (flag = pc_additem(p_sd, &item_tmp, amount,LOG_TYPE_SCRIPT)) )
											clif_additem(p_sd,0,0,flag);
									}
							}else
								if( (flag = pc_additem(p_sd, &item_tmp, amount,LOG_TYPE_SCRIPT)) )
									clif_additem(p_sd,0,0,flag);
...
/*===========================================
 * getitemparty(<item_id>,<amount>{,<party_id>,"<map_name>"});
 * by: CarlosHenrq
 *-------------------------------------------*/
BUILDIN_FUNC(getitemparty){
	struct map_session_data * sd = script_rid2sd(st);
	TBL_PC *p_sd;
	struct item item_tmp;
	struct item_data *item_data;
	int nameid,amount,party,flag,count,i,x,m;
	struct party_data *p;
	
	party = ((script_getnum(st,4)) ? script_getnum(st,4):sd->status.party_id);
	...
	if( (p = party_search(party)) != NULL ){
		if( (item_data = itemdb_exists(script_getnum(st,2))) != NULL ){
			...
			if( !itemdb_isstackable2(item_data) )
				amount = 1;
			for( i = 0; i < MAX_PARTY; i++ ){
				if( (p_sd = map_id2sd(p->party.member[i].account_id)) != NULL ){
					for( x = 0; x < count; x += amount ){
						if( !pet_create_egg(p_sd, nameid) ){
							memset(&item_tmp, 0, sizeof(item_tmp));
							item_tmp.nameid = nameid;
...
BUILDIN_FUNC(cartreset)
{
	...
	for (i = 0; i < MAX_CART; i++) {
		...
		//Logs (A)dmins items
			log_pick_pc(sd, LOG_TYPE_SCRIPT, sd->status.cart[i].nameid, -1, &sd->status.cart[i]);
	
			pc_cart_delitem(sd, i, sd->status.cart[i].amount, 0,LOG_TYPE_SCRIPT);
		}
	}
	return 0;
}
...
BUILDIN_FUNC(pkmode)
{
      TBL_PC *sd = script_rid2sd(st);
       ...
       if( type == 1 ) {
			...
			strcpy(output, "､ﾘｳ莇鰲｢鰓ﾊﾙ鞣ﾋﾁｴ PK");
			clif_disp_onlyself(sd, output, strlen(output));
       } else {																																				duel_leave(sd->duel_group, sd);
			strcpy(output, "､ﾘｳ莇鯱ﾍ｡ｨﾒ｡簍ﾁｴ PK");
			clif_disp_onlyself(sd, output, strlen(output));
       }
...
// <--- [zBuffer] List of mob control commands
	...
	BUILDIN_DEF(setdropbbbonus, "??"),
	BUILDIN_DEF(getdropbbbonus, "?"),
	
	/* Epoque's Expansion Pack */
	BUILDIN_DEF(getelement, "i"),
