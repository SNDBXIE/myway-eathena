...
// returns value to increase drops by.
int party_drop_increase(struct party_data* party, int base_rate){
	int a=0, i=0, tmp=0;
	// No point doing anything if value isnt set.
	if (battle_config.party_drop_bonus > 0){
		// Get size of party - offline players
		for (a = 0; a < MAX_PARTY; a ++){
			if (party->party.member[a].online){
				i++;
			}
		}
		a=0;
		// -1 so that we only apply on parties with 2 or more members
		i--;
		if (i > 0){
			int tmp = base_rate+((base_rate/(i+1))*((battle_config.party_drop_bonus*(i+1))/100));
