...
char motd_text[MOTD_LINE_SIZE][CHAT_SIZE_MAX]; // Message of the day buffer [Valaris]
...
void pc_setpvparea(struct map_session_data* sd, bool flag)
{
	if( sd && !sd->pvp )
	{
...
int pc_overheat(struct map_session_data *sd, int val)
{
	...
	skill = cap_value(pc_checkskill(sd,NC_MAINFRAME),0,4); // Ini gunanya ap ya??gmna cara ngubahnya??Level 1: 200 Level 2: 280 Level 3: 360 Level 4: 450 [ Info dari website ]
...
void pc_inventory_rentals(struct map_session_data *sd)
{
	...
	{ // Check for Rentals on Cart
		...
		{
			clif_rental_expired(sd->fd, i, sd->status.cart[i].nameid);
			pc_delitem(sd, i, sd->status.cart[i].amount, 1, 0, LOG_TYPE_OTHER);
		}
		else
		{
	...
	{ // Check for Rentals on Storage
		...
		{
			clif_rental_expired(sd->fd, i, sd->status.storage.items[i].nameid);
			pc_delitem(sd, i, sd->status.storage.items[i].amount, 1, 0, LOG_TYPE_OTHER);
		}
		else
		{
...
int pc_equippoint(struct map_session_data *sd,int n)
{
	...
	if(sd->inventory_data[n]->look == W_DAGGER	||
		...
		if(ep == EQP_HAND_R && (pc_checkskill(sd,AS_LEFT) > 0 || (sd->class_&MAPID_UPPERMASK) == MAPID_ASSASSIN || (sd->class_&MAPID_UPPERMASK) == MAPID_KAGEROUOBORO)) //Kagerou and Oboro can dual wield daggers. [Rytech]
			return EQP_ARMS;
	}
	return ep;
}
...
int pc_reg_received(struct map_session_data *sd)
{
	...
	pc_setglobalreg( sd, "VENDING_MODE", 0 );
	...
	for( i = 0; i < max; i++ )
	{
		if( autobonus[i].active != INVALID_TIMER )
		{
			if( restore && sd->state.autobonus&autobonus[i].pos )
			{
				...
				{
					int j;
					ARR_FIND( 0, EQI_MAX-4, j, sd->equip_index[j] >= 0 && sd->status.inventory[sd->equip_index[j]].equip == autobonus[i].pos );
					if( j < EQI_MAX-4 )
...
int pc_exeautobonus(struct map_session_data *sd,struct s_autobonus *autobonus)
{
	...
	{
		int j;
		ARR_FIND( 0, EQI_MAX-4, j, sd->equip_index[j] >= 0 && sd->status.inventory[sd->equip_index[j]].equip == autobonus->pos );
		if( j < EQI_MAX-4 )
...
int pc_bonus(struct map_session_data *sd,int type,int val)
{
	...
	// Epoque's Expansion Pack
		case SP_RACE:
			...
			{
				if (val < RC_FORMLESS || val > RC_MAX - 1) {
					ShowWarning("pc_bonus:SP_RACE: Invalid race (%d) specified.\n", val);
					break;
			}
				sd->battle_status.race = val;
			}
			break;
		case SP_NOSINGLEMAGIC:
			if (sd->state.lr_flag != 2)
				sd->special_state.no_single_magic += cap_value(sd->special_state.no_single_magic + val, 0, 100);
			break;
		case SP_NOAREAMAGIC:
			if (sd->state.lr_flag != 2)
				sd->special_state.no_area_magic += cap_value(sd->special_state.no_area_magic + val, 0, 100);
			break;
		case SP_SKILLTIME:
			if (sd->state.lr_flag != 2)
				sd->special_state.skill_time += cap_value(sd->special_state.skill_time + val, 0, 100);
			break;
		case SP_AVOIDTRAP:
			if (sd->state.lr_flag != 2)
				sd->special_state.avoid_trap = cap_value(sd->special_state.avoid_trap + val, 0, 100);
			break;
		case SP_MOVECAST:
			if (sd->state.lr_flag != 2)
				sd->special_state.skill_move = cap_value(sd->special_state.skill_move + val, 0, CHAR_MAX);
			break;
		case SP_HEALATTACK:
		if (sd->state.lr_flag != 2)
				sd->heal_attack |= cap_value(val, 0, 7);
			break;
		case SP_LASTSTAND:
			if (sd->state.lr_flag != 2)
		...
		case SP_MAXDAMAGE:
		if (sd->state.lr_flag != 2)
				sd->damage_type[0] += val;
			break;
		case SP_MINDAMAGE:
			if (sd->state.lr_flag != 2)
				sd->damage_type[1] += val;
			break;
		case SP_CRITATKMISS:
			if (sd->state.lr_flag != 2)
				sd->special_state.avoid_crit = cap_value(sd->special_state.avoid_crit + val, 0, 100);
			break;
		case SP_MAGICDAMAGERETURN2:
			if (sd->state.lr_flag != 2)
				sd->magic_damage_return2 += val;
			break;
		case SP_MDEFATKRATIO_RC:
			if (sd->state.lr_flag != 2)
			{
				if (val < 0 || val >= RC_MAX) {
					ShowWarning("pc_bonus: SP_MDEFATKRATIO_RC: Invalid race %d\n", val);
					break;
				}
			...
			break;
		case SP_MDEFATKRATIO_ELE:
			if (sd->state.lr_flag != 2)
			{
				if (val < 0 || val >= ELE_MAX) {
					ShowWarning("pc_bonus: SP_MDEFATKRATIO_ELE: Invalid element %d\n", val);
					break;
				}
	...
	case SP_ADD_ITEM_HEAL_RATE:
		...
		break;
	case SP_FIXED_CASTRATE:
		if(sd->state.lr_flag != 2)
			sd->fixed_castrate += val;
		break;
	case SP_FIXED_CAST:
		if(sd->state.lr_flag != 2)
			sd->fixed_cast += val;
		break;
	case SP_VARIABLE_CASTRATE:
		if(sd->state.lr_flag != 2)
			sd->variable_castrate +=val;
		break;
	case SP_VARIABLE_CAST:
		if(sd->state.lr_flag != 2)
			sd->variable_cast +=val;
		break;
	case SP_EMATK:
		if(sd->state.lr_flag != 2)
			sd->ematk += val;
...
int pc_bonus2(struct map_session_data *sd,int type,int type2,int val)
{
	...
	switch(type){
		//Epoque's Expansion Pack
		case SP_HPATKRATIO:
			sd->hp_ratio[type2] += val;
			break;
		case SP_SPATKRATIO:
			sd->sp_ratio[type2] += val;
			break;
		case SP_ELEDAMAGE:
			if (type2 < 0 || type2 >= ELE_MAX) {
				ShowWarning("pc_bonus2: SP_ELEDAMAGE: Invalid element %d\n", type2);
				break;
			}
			sd->ele_damage[type2] += val;
			break;
		case SP_SCDURATION:
			if (type2 < SC_COMMON_MIN || type2 >= SC_MAX) {
				ShowWarning("pc_bonus2: SP_SCDURATION: Invalid status %d\n", type2);
				break;
			}
			sd->sc_effect[type2].duration += val;
			break;
		case SP_SCDAMAGE:
			if (type2 < SC_COMMON_MIN || type2 >= SC_MAX) {
				ShowWarning("pc_bonus2: SP_SCDAMAGE: Invalid status %d\n", type2);
				break;
			}
			sd->sc_effect[type2].damage += val;
			break;
		case SP_SCRESIST:
			if (type2 < SC_COMMON_MIN || type2 >= SC_MAX ) {
				ShowWarning("pc_bonus2: SP_SCRESIST: Invalid status %d\n", type2);
				break;
			}
			sd->sc_effect[type2].resist += val;
			break;
		case SP_NOCONSUME:
			if (val == 0)
				val = NC_ALLFLAG;
			...
			if (type2&NC_ATTACK)
				sd->no_consume[1] += val;
			if (type2&NC_GEMSTONE)
				sd->no_consume[2] += val;
			if (type2&NC_SKILLSP)
				sd->no_consume[3] += val;
			break;
		case SP_MOVECAST:
			ARR_FIND(0, MAX_PC_BONUS, i, sd->move_skill[i].id == 0 || sd->move_skill[i].id == type2);

			if (i == MAX_PC_BONUS) {
				ShowWarning("pc_bonus2: SP_MOVECAST: Reached the maximum skill movement bonuses (%d)\n", MAX_PC_BONUS);
				break;
			}

			sd->move_skill[i].id = type2;
			sd->move_skill[i].move += val;
			break;
		case SP_FULLDODGE:
			if ((val&BF_WEAPONMASK) == 0) {
				ShowWarning("pc_bonus2: SP_FULLDODGE: No valid attack type given (%d)\n", val);
				break;
			}
			if ((val&BF_RANGEMASK) == 0)
				val = BF_LONG|BF_SHORT;
			if (val&BF_WEAPON) {
				sd->full_dodge[0].flag = (val&BF_RANGEMASK);
				sd->full_dodge[0].rate += type2;
			}
			if (val&BF_MAGIC) {
				sd->full_dodge[1].flag = (val&BF_RANGEMASK);
				sd->full_dodge[1].rate += type2;
			}
			if (val&BF_MISC) {
				sd->full_dodge[2].flag = (val&BF_RANGEMASK);
				sd->full_dodge[2].rate += type2;
			}
			break;
		case SP_CRITICALDODGE:
			...
			break;
		case SP_IGNOREGTB:
			sd->ignore_gtb.rate = cap_value(sd->ignore_gtb.rate + type2, 1, 1000);
			sd->ignore_gtb.damage += val;
			break;
	case SP_ADDELE:
		if(type2 >= ELE_MAX) {
	...
	case SP_FIXED_CASTRATE:
		...
		if(sd->skillcast[i].id == type2){
			sd->skillcast[i].val = max(sd->skillcast[i].val,val);
			sd->skillcast[i].casttype = 1;
		}else {
			sd->skillcast[i].id = type2;
			sd->skillcast[i].val = max(sd->skillcast[i].val,val);
			sd->skillcast[i].casttype = 1;
	...
	case SP_VARIABLE_CASTRATE:
		...
		if (i == ARRAYLENGTH(sd->skillcast))
		{	//Better mention this so the array length can be updated. [Skotlex]
			ShowDebug("run_script: bonus2 bVarCastRate reached it's limit (%d skills per character), bonus skill %d (+%d%%) lost.\n", ARRAYLENGTH(sd->skillcast), type2, val);
			break;
		}
		if(sd->skillcast[i].id == type2){
			sd->skillcast[i].val += val;
			sd->skillcast[i].casttype = 2;
		}else {
			sd->skillcast[i].id = type2;
			sd->skillcast[i].val = val;
			sd->skillcast[i].casttype = 2;
	...
	case SP_VARIABLE_CAST:
		...
		if (i == ARRAYLENGTH(sd->skillcast))
		{	//Better mention this so the array length can be updated. [Skotlex]
			ShowDebug("run_script: bonus2 bVarCast reached it's limit (%d skills per character), bonus skill %d (+%d%%) lost.\n", ARRAYLENGTH(sd->skillcast), type2, val);
			break;
		}
		if(sd->skillcast[i].id == type2){
			sd->skillcast[i].val += val;
			sd->skillcast[i].casttype = 3;
		}else {
			sd->skillcast[i].id = type2;
			sd->skillcast[i].val = val;
			sd->skillcast[i].casttype = 3;
	...
	case SP_COOLDOWNRATE:
		ARR_FIND(0, ARRAYLENGTH(sd->cooldownrate), i, sd->cooldownrate[i].id == 0 || sd->cooldownrate[i].id == type2);
		if(i == ARRAYLENGTH(sd->cooldownrate)) {
			ShowWarning("pc_bonus2: Reached max (%d) number of cooldownrate bonuses per character!\n", ARRAYLENGTH(sd->cooldownrate));
			break;
		}
		sd->cooldownrate[i].id   = type2;
		sd->cooldownrate[i].val += val;
		break;		
	case SP_COOLDOWN:
	...
	case SP_HP_GAIN_RACE_ATTACK:
		...
		break;
    case SP_SPCOST:
		ARR_FIND(0, ARRAYLENGTH(sd->spcost), i, sd->spcost[i].id == 0 || sd->spcost[i].id == type2);
		if(i == ARRAYLENGTH(sd->spcost)) {
			ShowWarning("pc_bonus2: Reached max (%d) number of spcost bonuses per character!\n", ARRAYLENGTH(sd->spcost));
			break;
		}
		sd->spcost[i].id   = type2;
		sd->spcost[i].val += val;
		break;        break;
	case SP_SPCOSTRATE:
		ARR_FIND(0, ARRAYLENGTH(sd->spcostrate), i, sd->spcostrate[i].id == 0 || sd->spcostrate[i].id == type2);
		if(i == ARRAYLENGTH(sd->spcostrate)) {
			ShowWarning("pc_bonus2: Reached max (%d) number of spcostrate bonuses per character!\n", ARRAYLENGTH(sd->spcostrate));
			break;
		}
		sd->spcostrate[i].id   = type2;
		sd->spcostrate[i].val += val;
		break;
	default:
...
int pc_bonus3(struct map_session_data *sd,int type,int type2,int type3,int val)
{
	...
	switch(type){
		...
		case SP_AREASKILL:
			if (sd->state.lr_flag != 2)
				{
					ARR_FIND(0, MAX_PC_BONUS, i, sd->area_skill[i].id == 0);

					if (i == MAX_PC_BONUS) {
						ShowWarning("pc_bonus3: SP_AREASKILL: Reached maximum area skills (%d)\n", MAX_PC_BONUS);
						break;
					}

					if (val == 0) {
						ShowWarning("pc_bonus3: SP_AREASKILL: Invalid range specified (%d)\n", val);
						break;
					}

					if (skill_get_splash(type2, 1) != 0) {
					ShowWarning("pc_bonus3: SP_AREASKILL: Specified skill already has area effect\n");
						break;
					}

					sd->area_skill[i].id = type2;
					sd->area_skill[i].rate = cap_value(type3, 1, SHRT_MAX);
					sd->area_skill[i].range = cap_value(val, -1, SHRT_MAX);
				}
			break;	

		case SP_SCEXCHANGE:
			if (type2 < SC_NONE || type2 >= SC_MAX) {
				ShowWarning("pc_bonus3: SP_SCEXCHANGE: Invalid initial status %d\n", type2);
				break;
			}
			if (type3 < SC_NONE || type3 >= SC_MAX) {
				ShowWarning("pc_bonus3: SP_SCEXCHANGE: Invalid exchange status %d\n", type3);
				break;
			}
			sd->sc_effect[type2].exchange = (enum sc_type)(type3 + 1);
			sd->sc_effect[type2].rate = val;
			break;

		case SP_SKILLGAINSP:
			{
				ARR_FIND(0, MAX_PC_BONUS, i, sd->sp_skill[i].id == 0);

				if (i == MAX_PC_BONUS) {
					ShowWarning("pc_bonus3: SP_SKILLGAINSP: Reached maximum SP bonus (%d)\n", MAX_PC_BONUS);
				break;
				}
				sd->sp_skill[i].id = type2;
				sd->sp_skill[i].rate = cap_value(type3, 1, 1000);
				sd->sp_skill[i].sp = (unsigned short)val;
			}
			break;

		case SP_HPDRAINRATIO:
			if (type2 < 0 || type2 >= RC_MAX) {
				ShowWarning("pc_bonus3: SP_HPDRAINRATIO: Invalid race given (%d)\n", type2);
				break;
			}
			sd->hp_drain[type2].amount = max(type3, sd->hp_drain[type2].amount);
			sd->hp_drain[type2].rate = max(val, sd->hp_drain[type2].rate);
			break;

		case SP_SPDRAINRATIO:
			if (type2 < 0 || type2 >= RC_MAX) {
				ShowWarning("pc_bonus3: SP_SPDRAINRATIO: Invalid race given (%d)\n", type2);
				break;
			}
			sd->sp_drain[type2].amount = max(type3, sd->sp_drain[type2].amount);
			sd->sp_drain[type2].rate = max(val, sd->sp_drain[type2].rate);
			break;
...
/// Get the number of party member that were found
/// Return -1 if there is no party
int pc_getpartymember(struct map_session_data* sd, int flag)
{
	struct party_data *p;
	int i, j=0;
	...
	for (i = 0; i < MAX_PARTY; i++){
		if (p->party.member[i].account_id) {
			if (flag && !map_charid2sd(p->party.member[i].char_id)) //Check for online player
				continue;
			j++;
		}
	}
...
int pc_additem(struct map_session_data *sd,struct item *item_data,int amount,e_log_pick_type log_type)
{
	...
	if(data->type == IT_CHARM) status_calc_pc(sd,0);
	return 0;
}
...
int pc_memo(struct map_session_data* sd, int pos)
{
	...
	// check mapflags+PVP Area by Mr.Postman
	if( sd->bl.m >= 0 && (map[sd->bl.m].flag.nomemo || map[sd->bl.m].flag.nowarpto) && battle_config.any_warp_GM_min_level > pc_isGM(sd) || map_getcell(sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP)) {
...
const char* job_name(int class_)
{
	...
	case JOB_GUILLOTINE_CROSS:
		return msg_txt(622 - JOB_RUNE_KNIGHT+class_);
	...
	case JOB_GUILLOTINE_CROSS_T:
		return msg_txt(622 - JOB_RUNE_KNIGHT_T+class_);
	...
	case JOB_SHADOW_CHASER:
		return msg_txt(628 - JOB_ROYAL_GUARD+class_);
	...
	case JOB_SHADOW_CHASER_T:
		return msg_txt(628 - JOB_ROYAL_GUARD_T+class_);
	...
	case JOB_BABY_CHASER:
		return msg_txt(639 - JOB_BABY_RUNE+class_);
...
int pc_need_status_point(struct map_session_data* sd, int type, int val)
{
	...
	if ( pc_getstat(sd,type) >= pc_maxparameter(sd))
		return 0;
...
void pc_respawn(struct map_session_data* sd, clr_type clrtype)
{
	...
	//PVP Area by Mr.Postman
	if( sd->pvp && battle_config.deathmatch )
	{
		int x, y;
...
int pc_dead(struct map_session_data *sd,struct block_list *src)
{
	...
	// pvp
	// disable certain pvp functions on pk_mode [Valaris]+PVP Area by Mr.Postman
	if( sd->pvp || map[sd->bl.m].flag.pvp && !battle_config.pk_mode && !map[sd->bl.m].flag.pvp_nocalcrank )
	{
		sd->pvp_point -= 5;
		sd->pvp_lost++;
		if( src && src->type == BL_PC )
		...
		//PVP Area by Mr.Postman
		if( sd->pvp_point < 0 || battle_config.deathmatch)
		{
...
int pc_calc_pvprank(struct map_session_data *sd)
{
	...
	if( sd->pvp ) {
		clif_pvpset( sd, sd->pvp_rank, sd->pvp_lastusers=m->pvp_user, 0 );
		ret = sd->pvp_rank;
	}
	else if(old!=sd->pvp_rank || sd->pvp_lastusers!=m->users) {		
...
//			ShowDebug("%s - Class %d: %u\n", type?"Job":"Base", job_id, max_level[job][type]);
	...
	battle_config.use_statpoint_table = k; //restore settin
