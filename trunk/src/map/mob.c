...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	for(i = 0; i < DAMAGELOG_SIZE && md->dmglog[i].id; i++) {
		...
		if (map[m].flag.nobaseexp || !md->db->base_exp || (sd && sd->sc.data[SC_TRICKDEAD] && battle_config.tickdead_noexp))	//checked skill tickdead
			base_exp = 0;
		else
			base_exp = (unsigned int)cap_value(md->db->base_exp * per * bonus/100. * map[m].bexp/100., 1, UINT_MAX);

		if (map[m].flag.nojobexp || !md->db->job_exp || md->dmglog[i].flag == MDLF_HOMUN || (sd && sd->sc.data[SC_TRICKDEAD] && battle_config.tickdead_noexp)) //Homun earned job-exp is always lost. + checked skill tickdead
			job_exp = 0;
		else
		...
		pc_setglobalreg(sd, "getBaseExp", base_exp);
		pc_setglobalreg(sd, "getJobExp", job_exp);
	}
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP; i++)
		{
			...
			if(mvp_sd) 
			{
            	...
            	if(dd->ann==1){
                    sprintf (anme, msg_txt(NULL,541), mvp_sd->status.name, md->name, dd->jname, (float)drop_rate/100);
                    //MSG: "'%s' won %s's %s (chance: %0.02f%%)"
		...
		if( !(map[m].flag.nomvploot || type&1) ) {
			...
			for(i = 0; i < MAX_MVP_DROP; i++) {
				...
				if(mvp_sd) 
				{
                    ...
                    if(dd->ann==1){
                    	sprintf (anme, msg_txt(NULL,541), mvp_sd->status.name, md->name, dd->jname, (float)temp/100);
                        //MSG: "'%s' won %s's %s (chance: %0.02f%%)"
                        intif_broadcast(anme,strlen(anme)+1,0);
					}
            	}
	...
	if( !rebirth ) { // Only trigger event on final kill
		...
		if( md->npc_event[0] && !md->state.npc_killmonster ) {
			...
			npc_script_event(mvp_sd, ((md->db->mexp) ? NPCE_KILLBOSSNPC:NPCE_KILLNPC)); // PCKillNPC [Lance] - PcKillBossNPC [CarlosHenrq]
		}
