...
//Defines the Bifrost mob groups for the status reductions [Ziu]
const int mob_bifrost[6] = { 2132, 2133, 2134, 2135, 2136, 2137 };
...
int mob_delayspawn(int tid, unsigned int tick, int id, intptr data)
{
	...
	if( md )
	{
		...
		if( md->killer && md->tombstone && battle_config.active_mvp_tombstone){
			...
			npc_tombstoneremove(tombnpc); // Tombstone [Mark]
		}
		mob_spawn(md);
	}
	return 0;
...
static void mob_item_drop(struct mob_data *md, struct item_drop_list *dlist, struct item_drop *ditem, int loot, int drop_rate, unsigned short flag)
{
	...
	if( sd
		...
		&& (battle_config.homunculus_autoloot?1:!flag) && !(md->status.mode&MD_BOSS)
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	int i,temp,count,pnum=0,m=md->bl.m,temp_rate;
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP; i++)
		{
			...
			if(mvp_sd && !(status->mode&MD_BOSS)){
				if (mvp_sd->drop_bonus > 0){
					temp_rate = (drop_rate+((drop_rate*mvp_sd->drop_bonus)/100));
				}
			}
			
			// Party Bonus to Drop Rate [WiseWarrior]
            if (mvp_sd && mvp_sd->status.party_id > 0 && !(status->mode&MD_BOSS)){
				if(temp_rate > 0){
					temp_rate = party_drop_increase(party_search(mvp_sd->status.party_id),temp_rate);
				} else {
					temp_rate = party_drop_increase(party_search(mvp_sd->status.party_id),drop_rate);
				}
			}
			if(temp_rate > 10000) temp_rate=9000;
			if(temp_rate < 0) temp_rate=0;

			// Increase drop rate if user has SC_ITEMBOOST
			if( sd && sd->sc.data[SC_ITEMBOOST] ) // now rig the drop rate to never be over 90% unless it is originally >90%.
			{
				if(temp_rate > 0 && temp_rate < 10000){
					temp_rate = max(temp_rate,cap_value((int)(0.5+temp_rate*(sd->sc.data[SC_ITEMBOOST]->val1)/100.),0,9000));
				} else {
			...
			if (temp_rate > 0){
				if (rand() % 10000 >= temp_rate){
					if (!(mvp_sd && mvp_sd->dropbb_bonus > 0 && rand() % 10000 < mvp_sd->dropbb_bonus && rand() % 10000 < temp_rate)) 
						continue;	//not drop
				}
			} else {
	...
	if(md->vd && pcdb_checkid(md->vd->class_))
...
/*==========================================
 * ?æ?Ê?à?Ì?æ?è?ª?«?Ì??v?Z?p(foreachinarea)
 *------------------------------------------*/
...
/*==========================================
 *MOBskill?©?ç?Y??skillid?Ìskillidx?ð?Ô?·
 *------------------------------------------*/
int mob_skillid2skillidx(int class_,int skillid)
{
...
static bool mob_readdb_mobavail(char* str[], int columns, int current)
{
	...
	if(mob_db(class_) == mob_dummy)	// ?l?ª?Ùí?È?ç???µ?È?¢B
	{
