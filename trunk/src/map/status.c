...
int status_check_skilluse(struct block_list *src, struct block_list *target, int skill_num, int skill_lv, int flag)
{
	...
	if( sc && sc->count )
	{
		...
		if (skill_num && //Do not block item-casted skills.
			...
			//Skill blocking.+ Guildwar Observe by Mr.Postman
			if (
...
int status_calc_elemental_(struct elemental_data *ed, bool first)
{
	...
	if( first )
	{
		// FIXME : Summoned Elemental's STATs are affected by the Caster痴 Base Level and STATs.
...
int status_change_start(struct block_list* bl,enum sc_type type,int rate,int val1,int val2,int val3,int val4, int tick,int flag)
{
	...
	switch(type)
	{
		...
		case SC_FORCEOFVANGUARD:
			...
			break;
		case SC_EXEEDBREAK:
			val1 *= 100; // ATK = [{(Skill Level x 100) + (Caster's Job Level x 10)} + (Weapon Weight x Weapon Level) x (Caster痴 Base Level / 100)] %
			if( sd )
			{	// Chars.
		...
		case SC_PRESTIGE: // Magic avoidance: [(Caster's INT + LUK) x (Skill Level / 20) x (Caster's Base Level / 200)] + (Skill Level) %
			...
			val1 *= 15; // DEF increase: [{(Skill Level x 15) + (Defending Aura Skill Level x 10)} x Caster痴 Base Level / 100]
			if( sd )
