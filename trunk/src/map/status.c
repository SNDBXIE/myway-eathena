...
int status_calc_pc_(struct map_session_data* sd, bool first)
{
	...
	for(i=0;i<EQI_MAX-1;i++) {
		...
		if(sd->inventory_data[index]->type == IT_WEAPON) {
			...
			// refine bonus[Cydh]
			if(refine_bonus[wlv].refine[sd->status.inventory[index].refine].script)
				run_script(refine_bonus[wlv].refine[sd->status.inventory[index].refine].script,0,sd->bl.id,0);
			...
			if(sd->inventory_data[index]->script) {
				...
				// refine bonus [Cydh]
				if(refine_bonus[REFINE_TYPE_ARMOR].refine[sd->status.inventory[index].refine].script)	
					run_script(refine_bonus[REFINE_TYPE_ARMOR].refine[sd->status.inventory[index].refine].script,0,sd->bl.id,0);
...
static int status_readdb_refine_bonus(void)
{
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if( refine < 0 || refine > MAX_REFINE ) 
		{
			ShowError("status_readdb_refine_bonus : Invalid item refine number '%s'.\n",atoi(str[1]));
			continue;
		}
