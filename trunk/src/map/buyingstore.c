...
{
	 int zeny = 0, temp = 0, num, itemid, idx;
	...
	{// itemlist: <index>.W <name id>.W <amount>.W
		...
		weight+= amount*sd->inventory_data[index]->weight;
		 ...
		 for( k = 0; k < MAX_INVENTORY; k++ )
			 if( pl_sd->status.inventory[k].nameid == pc_readglobalreg(pl_sd, "BUYINGSTORE_ITEM_CHANGE") )
				  temp += pl_sd->status.inventory[k].amount;

		  num = ( pc_readglobalreg(pl_sd, "BUYINGSTORE_MODE") == BUYINGSTORE_ITEM_MODE )  ? temp : pl_sd->buyingstore.zenylimit-zeny;

		   if( amount*pl_sd->buyingstore.items[listidx].price > num )
	...
	{// itemlist: <index>.W <name id>.W <amount>.W
		...
		// pay up
		if( pc_readglobalreg(pl_sd, "BUYINGSTORE_MODE") == BUYINGSTORE_ITEM_MODE )
		{
			  itemid = pc_readglobalreg(pl_sd, "BUYINGSTORE_ITEM_CHANGE");
			  idx = pc_search_inventory(pl_sd, itemid);

			   memset(&it, 0, sizeof(it));
			   it.nameid = itemid;
			   it.identify = 1;

			   pc_delitem(pl_sd, idx, zeny, 0, 0, LOG_TYPE_BUYING_STORE);
			   pc_additem(sd, &it, zeny, LOG_TYPE_BUYING_STORE);
		   } else {
