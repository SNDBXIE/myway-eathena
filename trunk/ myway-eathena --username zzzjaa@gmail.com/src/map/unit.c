...
static int unit_walktoxy_timer(int tid, unsigned int tick, int id, intptr data)
{
	...
	if(sd) {
		...
		if( !sd->pvp && map_getcell(bl->m, bl->x, bl->y, CELL_CHKPVP) )
			pc_setpvparea(sd, 0);
		
		else if( sd->pvp && !map_getcell(bl->m, bl->x, bl->y, CELL_CHKPVP) )
			pc_setpvparea(sd, 1);
	} else if (md) {
		if( map_getcell(bl->m,x,y,CELL_CHKNPC) ) {
...
int unit_skilluse_id2(struct block_list *src, int target_id, short skill_num, short skill_lv, int casttime, int castcancel)
{
	...
	if(sd) {
		if( skill_get_inf2(skill_num)&INF2_CHORUS_SKILL )
		{
			...
			{
				clif_skill_fail(sd,skill_num, 0x11, 0, 0);
				return 0;
			}
		}
		else
	...
	if( casttime > 0 )
	{
		...
		if( sd )
		{
			if (pc_checkskill(sd,SA_FREECAST) > 0 || skill_num == LG_EXEEDBREAK)
...
int unit_skilluse_pos2( struct block_list *src, short skill_x, short skill_y, short skill_num, short skill_lv, int casttime, int castcancel)
{
	...
	// "WHY IS IT HERE": pneuma cannot be cancelled past this point, the client displays the animation 
...
static int unit_attack_timer_sub(struct block_list* src, int tid, unsigned int tick)
{
	...
	if( status_isdead(src) || status_isdead(target) ||
		battle_check_target(src,target,BCT_ENEMY) <= 0 || !status_check_skilluse(src, target, 0, 0, 0) )
...
int unit_skillcastcancel(struct block_list *bl,int type)
{
	...
	if( sd )
	{
		if (pc_checkskill(sd,SA_FREECAST) > 0 || skill == LG_EXEEDBREAK){
