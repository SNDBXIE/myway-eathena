...
static int aspd_penalties[CLASS_COUNT][4];
...
static int thirdhp[CLASS_COUNT][MAX_LEVEL];
static int thirdsp[CLASS_COUNT][MAX_LEVEL];
...
void initChangeTables(void)
{
	...
	set_sc( HP_ASSUMPTIO         , SC_ASSUMPTIO       , SI_ASSUMPTIO_2       , SCB_DEF|SCB_DEF2|SCB_MDEF|SCB_MDEF2 );
	...
	set_sc( WM_GLOOMYDAY                  , SC_GLOOMYDAY               , SI_GLOOMYDAY                 , SCB_FLEE|SCB_ASPD|SCB_SPEED );
	set_sc( WM_SONG_OF_MANA               , SC_SONGOFMANA              , SI_SONGOFMANA                , SCB_NONE );
	set_sc( WM_DANCE_WITH_WUG             , SC_DANCEWITHWUG            , SI_DANCEWITHWUG              , SCB_ASPD|SCB_WATK );
	...
	set_sc( SO_FIRE_INSIGNIA     , SC_FIRE_INSIGNIA   , SI_FIRE_INSIGNIA   , SCB_MATK|SCB_BATK|SCB_WATK );
	set_sc( SO_WATER_INSIGNIA    , SC_WATER_INSIGNIA  , SI_WATER_INSIGNIA  , SCB_WATK );
	set_sc( SO_WIND_INSIGNIA     , SC_WIND_INSIGNIA   , SI_WIND_INSIGNIA   , SCB_WATK|SCB_ASPD );
	set_sc( SO_EARTH_INSIGNIA    , SC_EARTH_INSIGNIA  , SI_EARTH_INSIGNIA  , SCB_WATK|SCB_MDEF|SCB_DEF|SCB_MAXHP|SCB_MAXSP|SCB_WATK );
	
	set_sc( GN_CARTBOOST                  , SC_GN_CARTBOOST, SI_CARTSBOOST                 , SCB_BATK|SCB_SPEED );
	...
	set_sc( MH_GOLDENE_FERSE	 , SC_GOLDENE_FERSE   , SI_GOLDENE_FERSE	, SCB_SPEED|SCB_FLEE );
	...
	StatusIconChangeTable[SC_CRYSTALIZE] = SI_COLD;
	...
	StatusIconChangeTable[SC_GUARANA_CANDY] |= SI_GUARANA_CANDY;
	...
	StatusChangeFlagTable[SC_GUARANA_CANDY] |= SCB_ASPD;
	StatusChangeFlagTable[SC_BURNING] |= SCB_MDEF|SCB_MDEF2;
...
//		return 0; //Cannot damage a bl not on a map, except when "charging" hp/sp
	...
	// Chance to leave 1 HP when attacked to kill
...
int status_check_skilluse(struct block_list *src, struct block_list *target, int skill_num, int skill_lv, int flag)
{
	...
	if( sc && sc->count )
	{
		...
		if (skill_num && //Do not block item-casted skills.
			...
			//Skill blocking.+ ÃÐººà¢éÒªÁ¡ÔÅÇÍÃì by Mr.Postman
			if (
				...
				(sc->data[SC_NOCHAT] && sc->data[SC_NOCHAT]->val1&MANNER_NOSKILL) || sc->data[SC_AUDIENCE]
	...
	if( sc && sc->option )
	{
		...
		if (sc->option&OPTION_CHASEWALK && skill_num != ST_CHASEWALK)
			return 0;
		if (sc->option&OPTION_RIDING_WUG && src->type == BL_PC && ((TBL_PC*)src)->skillitem != skill_num)//Only usable skill while riding warg.
			switch( skill_num )
			{
		...
		if( sc->option&OPTION_MADO && src->type == BL_PC && ((TBL_PC*)src)->skillitem != skill_num && battle_config.mado_skills == 0)
		{
			switch( skill_num )
...
int status_base_amotion_pc(struct map_session_data* sd, struct status_data* status)
{
	...
	// Base ASPD
	amotion = aspd_base[pc_class2idx(sd->status.class_)][0];
	...
	if (sd->weapontype2) {
		switch(sd->weapontype2) {
			case W_DAGGER:
				amotion += aspd_penalties[pc_class2idx(sd->status.class_)][1];
				break;
			case W_1HSWORD:
				amotion += aspd_penalties[pc_class2idx(sd->status.class_)][2];
				break;
			case W_1HAXE:
				amotion += aspd_penalties[pc_class2idx(sd->status.class_)][3];
				break;
		}
	}
	if( sd->sc.count ) {
...
static unsigned short status_base_atk(struct block_list *bl, const struct status_data *status)
{
	...
	batk  = (status->luk*4 + status_get_lv(bl)*3) / 12;
	...
	batk += (flag ? status->dex : status->str) + (flag ? status->str : status->dex)/5;

	return cap_value(batk, 0, USHRT_MAX);
}

static inline unsigned short status_base_matk(const struct status_data* status, unsigned short level)
{
...
void status_calc_misc(struct block_list *bl, struct status_data *status, int level)
{
	...
	status->smatk += status_base_matk(status, level);

	status->matk_min = status->smatk;
	status->matk_max = status->smatk;
...
int status_calc_mob_(struct mob_data* md, bool first)
{
	...
	{	//Max HP setting from Summon Flora/marine Sphere
		...
		{	// different levels of HP according to skill level
			...
			/* // FIXME : Silver Sniper Max HP = (Caster's MHP / 3) + (Skill Level x 1000) + (Caster's Level x 12)
				  FIXME : Magic Decoy's Max HP = (Caster's MSP x 4) + (Skill Level x 1000) + (Caster's Base Level x 12)
				status->max_hp = (sd->status.max_hp / 3) + (ud->skilllv * 1000) + (sd->status.base_level * 12);
			if( ud->skillid == NC_MAGICDECOY )
				status->max_hp = (sd->status.max_sp / 4) + (ud->skilllv * 1000) + (sd->status.base_level * 12);
			*/
...
{
	int val, level;
	unsigned int job   = pc_jobid2mapid(sd->status.class_);
	unsigned int class_ = pc_class2idx(sd->status.class_);
	level = sd->status.base_level;
	if (sd->class_&JOBL_THIRD &&
		job != MAPID_SUPER_NOVICE_E && job != MAPID_SUPER_BABY_E &&
		job != MAPID_KAGEROUOBORO)
	{
		val = thirdhp[class_][cap_value(level-99, 0, 51)];
	}
	else
	{
		val = 35 + level*hp_coefficient2[class_]/100 + hp_sigma_val[class_][level];
	}
...
static unsigned int status_base_pc_maxsp(struct map_session_data* sd, struct status_data *status)
{
	...
	if (sd->class_&JOBL_THIRD &&
		...
		job != MAPID_KAGEROUOBORO)
	{
		val = thirdsp[class_][cap_value(level-99, 0, 51)];
	}
	else
	{
		val = 10 + level * sp_coefficient[class_] / 100;
	}
...
int status_calc_pc_(struct map_session_data* sd, bool first)
{
	...
	memset (sd->param_bonus, 0, sizeof(sd->param_bonus)
		...
		// Epoque's Expansion Pack
		+ sizeof(sd->ele_damage)
		+ sizeof(sd->hp_ratio)
		+ sizeof(sd->no_consume)
		+ sizeof(sd->sp_ratio)
		+ sizeof(sd->damage_type)
		);
	...
	memset(&sd->autospell, 0, sizeof(sd->autospell)
		...
		+ sizeof(sd->subele2)
		+ sizeof(sd->cooldown)
		+ sizeof(sd->cooldownrate)
		+ sizeof(sd->spcost)
		+ sizeof(sd->spcostrate)
		// Epoque's Expansion Pack
		+ sizeof(sd->sc_effect)
		+ sizeof(sd->area_skill)
		+ sizeof(sd->sp_skill)
		+ sizeof(sd->move_skill)
		+ sizeof(sd->critical_dodge)
		+ sizeof(sd->hp_drain)
		+ sizeof(sd->sp_drain)
		+ sizeof(sd->full_dodge)
		+ sizeof(sd->ignore_gtb)
	);
	...
	memset (&sd->atk_rate, 0,sizeof(sd->atk_rate)
		...
		// Epoque's Expansion Pack
		+ sizeof(sd->heal_attack)
		+ sizeof(sd->magic_damage_return2)
		);
	...
	for(i=0;i<EQI_MAX-4;i++) {
		...
		if(sd->inventory_data[index]->type == IT_WEAPON) {
			...
			wa->atk2  = r*refinebonus[wlv][0];
			if (sd->status.weapon != W_BOW) { //Refine Bow tidak menambah MATK
				wa->matk = r*refinebonus[wlv][0];
				if (sd->status.weapon == W_2HSTAFF)
					wa->matk >>= 2; //Bonus MATK dibagi 2 jika memakai 2 Handed Rod
			}
	...
	if(sd->equip_index[EQI_AMMO] >= 0) { //Ammunition
		index = sd->equip_index[EQI_AMMO];
		if(sd->inventory_data[index]) {
			sd->arrow_atk += sd->inventory_data[index]->atk;
			sd->state.lr_flag = 2;
			if (sd->inventory_data[index]->look != A_THROWWEAPON) //Not a Throwable Items
				run_script(sd->inventory_data[index]->script,0,sd->bl.id,0);
			sd->state.lr_flag = 0;
	...
	{
		for (i = 0; SC_MAX > i; i++)
			if (sd->sc_effect[i].bonus && sc->data[i])
				run_script(sd->sc_effect[i].bonus,0,sd->bl.id,0);
	}
...
// ----- EQUIPMENT-DEF CALCULATION -----
	...
	if( pc_isriding(sd,OPTION_MADO) && pc_checkskill(sd, NC_MAINFRAME)>0)
...
// Unlike other stats, ASPD rate modifiers from skills/SCs/items/etc are first all added together, then the final modifier is applied
	...
	if( (skill=pc_checkskill(sd,SA_ADVANCEDBOOK)) > 0 && sd->status.weapon == W_BOOK )
		status->aspd_rate += skill * 5;
	if( (skill = pc_checkskill(sd,SG_DEVIL)) > 0 && !pc_nextjobexp(sd) )
		status->aspd_rate += skill * 30;
	if( (skill=pc_checkskill(sd,GS_SINGLEACTION)) > 0 && (sd->status.weapon >= W_REVOLVER && sd->status.weapon <= W_GRENADE) )
		status->aspd_rate += (skill+1) * 5;
	if( pc_isriding(sd,OPTION_RIDING) )
		status->aspd_rate -= pc_checkskill(sd,KN_CAVALIERMASTERY) < 5 ? (5 - pc_checkskill(sd,KN_CAVALIERMASTERY)) * 100 : 0;
	if( pc_isriding(sd,OPTION_RIDING_DRAGON) && (sd->class_&JOBL_THIRD) && (skill = pc_checkskill(sd,RK_DRAGONTRAINING)) > 0 )
		status->aspd_rate -= pc_checkskill(sd,RK_DRAGONTRAINING) < 5 ? (5 - pc_checkskill(sd,RK_DRAGONTRAINING)) * 100 : 0;
...
int status_calc_elemental_(struct elemental_data *ed, bool first)
{
	...
	if( first )
	{
		// FIXME : Summoned Elemental's STATs are affected by the Caster’s Base Level and STATs.
...
static short status_calc_aspd_multiplier(struct block_list *,struct status_change *,int);
...
// if(flag&SCB_RANGE)
	...
	if (flag&SCB_MATK) {
		//FIXME: Bagaimana Kalkulasi MATK Monster/Homunculus?
		if (bl->type&BL_PC) {
			int wlvl;

			//Refresh MATK Value
			status->smatk = status_base_matk(status, status_get_lv(bl));

			status->wmatk = sd->wmatk + sd->ematk + sd->battle_status.lhw.matk + sd->battle_status.rhw.matk;

			if (sd->equip_index[EQI_HAND_R] &&
				sd->inventory_data[sd->equip_index[EQI_HAND_R]] &&
				sd->inventory_data[sd->equip_index[EQI_HAND_R]]->type == IT_WEAPON)
				wlvl = sd->inventory_data[sd->equip_index[EQI_HAND_R]]->wlv;
			else
				wlvl = 1;

			if (sd->wmatk_rate != 100) {
				//Bonuses from bWMatkRate
				status->wmatk = status->wmatk * sd->wmatk_rate/100;
			}

			status->smatk = status_calc_matk(bl, sc, status->smatk);

			status->matk_min = status->smatk + status->wmatk - status->wmatk*wlvl/10 + ((sd->status.weapon != W_BOW && (sd->right_weapon.overrefine || sd->left_weapon.overrefine)) ? 1 : 0);
			status->matk_max = status->smatk + status->wmatk + status->wmatk*wlvl/10 + ((sd->status.weapon != W_BOW && (sd->right_weapon.overrefine || sd->left_weapon.overrefine)) ? ((sd->right_weapon.overrefine + sd->left_weapon.overrefine) / (sd->status.weapon == W_2HSTAFF ? 2 : 1)) : 0);
			...
			status->matk_min = status_calc_matk(bl, sc, status_base_matk(status, status_get_lv(bl)));
			status->matk_max = status_calc_matk(bl, sc, status_base_matk(status, status_get_lv(bl)));
		}
		...
		if (bl->type&BL_HOM && battle_config.hom_setting&0x20) { //Homunculus
			status->matk_max = status->smatk;
			status->matk_min = status->smatk;
		}
	...
	if(flag&SCB_ASPD) {
		...
		{
			//JobASPD
			amotion = status_base_amotion_pc(sd,status) * 100;

			//PenaltyASPD & StatASPD
			amotion += ((amotion > 14500) ? 100 - ( amotion - 14400 ) / 50 : 96) * ((int)(sqrt( (double)((status->agi * 9.9987) + (status->dex * 0.1922))) * 100)) / 100;

			//Shield Penalty
			if (sd->equip_index[EQI_HAND_L] &&
				sd->inventory_data[sd->equip_index[EQI_HAND_L]] &&
				sd->inventory_data[sd->equip_index[EQI_HAND_L]]->type == IT_ARMOR) {
					amotion += aspd_penalties[pc_class2idx(sd->status.class_)][0] * 100;
			}

			//CorrectionASPD
			if (status->agi < 205)
				amotion -= (int)((double)(sqrt(205.0) - sqrt((double)status->agi)) * 10000 / 715 );

			//Raw Calculation
			amotion = 20000 - ( 20000 - amotion ) * (100 - status_calc_aspd_multiplier(bl, sc, b_status->aspd_rate) / 10) / 100;

			//Equipment Calculation
			amotion += (19500 - amotion) * b_status->aspd_rate2 / 1000;
			amotion += sd->aspd_add * 10;
		...
		else if( bl->type&BL_HOM )
		{
			...
			status->aspd_rate = status_calc_aspd_multiplier(bl, sc, b_status->aspd_rate);
			
			if(status->aspd_rate != 0)
				amotion = amotion*(1000+status->aspd_rate)/1000;
		...
		else // mercenary, elemental and mobs
		{
			...
			temp = b_status->adelay*(1000+status->aspd_rate)/1000;
...
void status_calc_bl_(struct block_list* bl, enum scb_flag flag, bool first)
{
	...
	if( bl->type == BL_PC )
	{
		...
		if(b_status.wmatk != status->wmatk)
			clif_updatestatus(sd,SP_MATK1);
		if(b_status.rhw.matk != status->rhw.matk || b_status.lhw.matk != status->lhw.matk || b_status.wmatk != status->wmatk)
...
static unsigned short status_calc_agi(struct block_list *bl, struct status_change *sc, int agi)
{
	...
	if(sc->data[SC_MARSHOFABYSS])
		agi -= agi * (bl->type == BL_PC ? 3 * sc->data[SC_MARSHOFABYSS]->val1 : 6 * sc->data[SC_MARSHOFABYSS]->val1) /100;
...
static unsigned short status_calc_dex(struct block_list *bl, struct status_change *sc, int dex)
{
	...
	if(sc->data[SC_MARSHOFABYSS])
		dex -= dex * (bl->type == BL_PC ? 3 * sc->data[SC_MARSHOFABYSS]->val1 : 6 * sc->data[SC_MARSHOFABYSS]->val1) /100;
...
//		batk -= batk * 25/100;
	...
	if(sc->data[SC_GT_CHANGE])
		batk += batk * (((status_get_dex(bl)/4)+(status_get_str(bl)/2)*sc->data[SC_GT_CHANGE]->val1/5)) / 100;
	...
	if(bl->type == BL_ELEM
		...
		batk += batk * 20 / 100;
	if(sc->data[SC_EDP] && battle_config.edp_renewal)
...
static unsigned short status_calc_watk(struct block_list *bl, struct status_change *sc, int watk)
{
	...
	if(sc->data[SC_DRUMBATTLE])
		watk += 100 + sc->data[SC_DRUMBATTLE]->val2;
	...
	if( sc->data[SC_DANCEWITHWUG] )
		watk += 2 * sc->data[SC_DANCEWITHWUG]->val1 * sc->data[SC_DANCEWITHWUG]->val2;
	if((sc->data[SC_FIRE_INSIGNIA] && sc->data[SC_FIRE_INSIGNIA]->val1 == 2)
		...
		watk += watk * 10 /100;
	if( sc->data[SC_EDP] && sc->data[SC_EDP]->val1 > 1 && battle_config.edp_renewal )
...
static signed short status_calc_def2(struct block_list *bl, struct status_change *sc, int def2)
{
	...
	if( sc->data[SC_GT_REVITALIZE] )
		def2 += def2 * ( (status_get_vit(bl) / 4) * sc->data[SC_GT_REVITALIZE]->val1 ) / 100;
...
static signed short status_calc_mdef(struct block_list *bl, struct status_change *sc, int mdef)
{
	...
	if(sc->data[SC_GT_CHANGE])
		mdef -= mdef * ((200/status_get_int(bl))*sc->data[SC_GT_CHANGE]->val1) / 100;
	if(sc->data[SC_WATER_BARRIER])
	...
	if( sd && sd->ud.skilltimer != INVALID_TIMER )
 	{
		...
		if (sd->ud.skillid == LG_EXEEDBREAK || pc_checkskill(sd,SA_FREECAST) > 0){
		...
		ARR_FIND(0, MAX_PC_BONUS, i, sd->move_skill[i].id == sd->ud.skillid);

		if (i < MAX_PC_BONUS )
			i = sd->move_skill[i].move + sd->special_state.skill_move;
		else
			i = sd->special_state.skill_move;

		// 101% = 101% - 1% = 100% (double walk speed when set to 1% speed)
		if (i)	speed_rate += (101 - i);
	}
	else
	{
		speed_rate = 100;
		...
		//GetMoveSlowValue()
		{
			...
			else
			{
				...
				if( sc->data[SC_SLOWDOWN] )
					val = max( val, sc->data[SC_SLOWDOWN]->val2 );
				if( sc->data[SC_GATLINGFEVER] )
					val = max( val, 100 );
				...
				if( sc->data[SC_FREEZING] )
					val = max( val, 70 );
				if( sc->data[SC_MARSHOFABYSS] )
					val = max( val, 10 * sc->data[SC_MARSHOFABYSS]->val1 );
...
/// ASPD Multiplier dari Status Change
/// 1000 = 100%
static short status_calc_aspd_multiplier(struct block_list *bl, struct status_change *sc, int aspd_rate)
{
	int ASPDch = 0;
	if (!sc || !sc->count)
		return cap_value(aspd_rate,SHRT_MIN,SHRT_MAX);
	...
	if(!sc->data[SC_QUAGMIRE] && !sc->data[SC_DECREASEAGI])
	{
		...
		if (sc->data[SC_ADRENALINE])
			aspd_rate += sc->data[SC_ADRENALINE]->val3;
		if (sc->data[SC_ADRENALINE2])
			aspd_rate += sc->data[SC_ADRENALINE2]->val3;
		if (sc->data[SC_ONEHAND]) {
			aspd_rate += sc->data[SC_ONEHAND]->val2;
			ASPDch = 1;
		}
		if (sc->data[SC_SPEARQUICKEN]) {
	...
	if (bl->type == BL_PC && pc_checkskill((TBL_PC*)bl,SG_DEVIL) > 0 && !pc_nextjobexp((TBL_PC*)bl))
		ASPDch = 1;
	if (sc->data[SC_ASSNCROS] && ASPDch == 0)
		if( bl->type == BL_PC ) {
			if (( ((TBL_PC*)bl)->status.weapon > W_GRENADE || ((TBL_PC*)bl)->status.weapon < W_REVOLVER ) &&
				  ((TBL_PC*)bl)->status.weapon != W_BOW)
				aspd_rate += sc->data[SC_ASSNCROS]->val1;
		}
		else
			aspd_rate += sc->data[SC_ASSNCROS]->val1;

	//Status yang menambah ASPD
	if (sc->data[SC_STAR_COMFORT])
		aspd_rate += sc->data[SC_STAR_COMFORT]->val2;
	if (sc->data[SC_GATLINGFEVER])
		aspd_rate += sc->data[SC_GATLINGFEVER]->val2;
	if (sc->data[SC_GT_CHANGE])
		aspd_rate += 10 * sc->data[SC_GT_CHANGE]->val2;
	if (sc->data[SC_MERC_QUICKEN])
		aspd_rate += sc->data[SC_MERC_QUICKEN]->val2;
	if (sc->data[SC_MADNESSCANCEL])
		aspd_rate += 200;
	if (sc->data[SC_BERSERK])
		aspd_rate += 200;
	if (sc->data[SC_SWINGDANCE])
		aspd_rate += 10 * sc->data[SC_SWINGDANCE]->val2;
	if (sc->data[SC_DANCEWITHWUG])
		aspd_rate += 10 * sc->data[SC_DANCEWITHWUG]->val3;
	if (sc->data[SC_WIND_INSIGNIA] && sc->data[SC_WIND_INSIGNIA]->val1 == 1)
		aspd_rate += 100;
	if (sc->data[SC_GOLDENE_FERSE])
		aspd_rate += 10 * sc->data[SC_GOLDENE_FERSE]->val3;
	if (sc->data[SC_FLEET])
		aspd_rate += sc->data[SC_FLEET]->val2;

	//Status yang mengurangi ASPD
	if (sc->data[SC_STEELBODY])
		aspd_rate -= 250;
	if (sc->data[SC_DEFENDER])
	...
	if (sc->data[SC__INVISIBILITY])
		aspd_rate -= sc->data[SC__INVISIBILITY]->val2;
	if (sc->data[SC__GROOMY])
		aspd_rate -= 10 * sc->data[SC__GROOMY]->val2;
	if (sc->data[SC_GLOOMYDAY])
	...
	if (sc->data[SC_ASPDPOTION3])		//???
		aspd_rate += sc->data[SC_ASPDPOTION3]->val2;
	else if (sc->data[SC_ASPDPOTION2])	//Berserk
		aspd_rate += sc->data[SC_ASPDPOTION2]->val2;
	else if (sc->data[SC_ASPDPOTION1])	//Awak
		aspd_rate += sc->data[SC_ASPDPOTION1]->val2;
	else if (sc->data[SC_ASPDPOTION0])	//Concent
		aspd_rate += sc->data[SC_ASPDPOTION0]->val2;

	//Other
	if (sc->data[SC_FIGHTINGSPIRIT])
		aspd_rate += sc->data[SC_FIGHTINGSPIRIT]->val3;
	if (sc->data[SC_GUARANA_CANDY])
		aspd_rate += 100;
	if( sc->data[SC_MELON_BOMB] )
	...
	return (short)cap_value(aspd_rate,SHRT_MIN,SHRT_MAX);
}
...
static unsigned int status_calc_maxhp(struct block_list *bl, struct status_change *sc, unsigned int maxhp)
{
	...
	if(sc->data[SC_INSPIRATION])
		maxhp += maxhp * (5 * sc->data[SC_INSPIRATION]->val1) / 100 + (sc->data[SC_INSPIRATION]->val1 * 600);
...
unsigned char status_calc_attack_element(struct block_list *bl, struct status_change *sc, int element)
{
	...
	if(sc->data[SC_WATERWEAPON] || sc->data[SC_TIDAL_WEAPON_OPTION] || sc->data[SC_TIDAL_WEAPON] || (sc->data[SC_WATER_INSIGNIA] && sc->data[SC_WATER_INSIGNIA]->val1 == 2) )
		return ELE_WATER;
...
// Epoque's Expansion Pack
int status_isimmune(struct block_list *bl, int skill_num)
{
	struct status_change *sc =status_get_sc(bl);
	if (sc && sc->data[SC_HERMODE])
		return 100;

	if (bl->type == BL_PC && skill_num > 0)
	{
		struct map_session_data* sd = (struct map_session_data*)bl;

		if (skill_get_casttype(skill_num) == CAST_GROUND && sd->special_state.no_area_magic >= battle_config.gtb_sc_immunity)
			return sd->special_state.no_area_magic;
		if (skill_get_casttype(skill_num) == CAST_DAMAGE && sd->special_state.no_single_magic >= battle_config.gtb_sc_immunity)
			return sd->special_state.no_single_magic;
	}
...
int status_get_sc_def(struct block_list *bl, enum sc_type type, int rate, int tick, int flag)
{
	...
	// To simplify things by a lot, there should be a renewal boolean statement to be put over this whole switch statement as well as making a
	// separate switch statement only for renewal [Inquisetor90]
	// TODO: BaseChance * (100 - ImmunityStat - BaseLevelDifference^2/2)/100 * EquipResist for all renewal classical status ailments
	// http://translate.google.co.kr/translate?hl=ko&sl=ko&tl=en&u=http://reversei.egloos.com/2670787
	// http://forums.irowiki.org/showpost.php?p=774047&postcount=2725
		switch (type)
		{
		case SC_STONE:
		case SC_FREEZE:
		...
		case SC_BLEEDING:
			sc_def = status->agi;
		case SC_SLEEP:
			// Duration is affected by int whereas chance is affected by agi. [Inquisetor90]
			sc_def = status->agi;
			tick_def = status->int_;
		case SC_STUN:
		case SC_POISON:
		case SC_DPOISON:
			// Stun is not warg bite status.
			sc_def = status->vit;
			break;
		case SC_BLIND:
		case SC_SILENCE:
		...
		case SC_ANKLE:
			if(status->mode&MD_BOSS) // Lasts 5 times less on bosses
				tick /= 5;
			tick -= 1000 * (status->agi / 10); // Every 10 agi reduces the duration by 1 second [Inquisetor90]			
			break;
		case SC_MAGICMIRROR:
		case SC_ARMORCHANGE:
			...
			break;
		case SC_DEEPSLEEP:
			tick -= 1000 * ((status_get_lv(bl)/20 + status->int_) / 20);
			tick = max(tick,5000); // Minimum Duration 5s.
			break;
		case SC_VOICEOFSIREN:
			tick -= 1000 * ((status_get_lv(bl)/10 + (sd? sd->status.job_level : 50)) / 5);
			break;
		case SC_ADORAMUS:
			if (sd) tick>>=1; //Half duration for players
		case SC_WHITEIMPRISON:
			...
			break;
		case SC_STASIS:
			tick -= 1000 * ((status->dex + status->vit) / 20);
			break;
		case SC_BURNING:
		...
		case SC_MARSHOFABYSS:
			tick += 25000 - (1000*((status->int_ + status->luk) / 20));
			break;
		case SC_OBLIVIONCURSE:
			sc_def += status->int_ / 125 * 10;
		case SC_PARALYSE:
		case SC_TOXIN:
		case SC_VENOMBLEED:
		...
		case SC_PYREXIA:
		case SC_LEECHESEND:
			if(sd && pc_checkskill(sd,GC_RESEARCHNEWPOISON)>0)
				sc_def += pc_checkskill(sd,GC_RESEARCHNEWPOISON)*5;
			else
				sc_def = 0;
			break;
		case SC_ELECTRICSHOCKER:
			tick -= ((status->vit+status->agi)/10) * 700;
			break;
		case SC_CRYSTALIZE:
			tick -= 1000 * ((status->vit/10)+(status_get_lv(bl)/50));
			break;
		case SC_VACUUM_EXTREME:
			tick -= 100 * status->str/20;
			break;
		case SC_MANDRAGORA:
	...
	// Epoque's Expansion Pack
	if (sd && sd->sc_effect[type].resist != 0)
		sc_def += sd->sc_effect[type].resist;
...
int status_change_start(struct block_list* bl,enum sc_type type,int rate,int val1,int val2,int val3,int val4, int tick,int flag)
{
	...
	// Epoque's Expansion Pack
	if (sd && sd->sc_effect[type].exchange > 0 && sd->sc_effect[type].rate > rand()%100)
		type = (enum sc_type)(sd->sc_effect[type].exchange - 1);
	...
	// Epoque @ Status Duration
	if (sd && sd->sc_effect[type].duration != 0)
		tick += tick * sd->sc_effect[type].duration / 100;
	...
	case SC_REFRESH:
		...
		status_change_end(bl, SC_BLIND,  INVALID_TIMER );
		status_change_end(bl, SC_CRITICALWOUND,  INVALID_TIMER );
	...
	if( (sce = sc->data[type]) )
	{
		switch( type )
		{
			...
			case SC__UNLUCKY:
			case SC_CHAOS:
			// ÃÐººà¢éÒªÁ¡ÔÅÇÍÃì by Mr.Postman
			case SC_AUDIENCE:
				return 0;
			case SC_COMBO: 
			case SC_DANCING:
	...
	switch(type)
	{
		...
		case SC_EXPLOSIONSPIRITS:
			...
			break;
		case SC_ASPDPOTION0: //Concentration Potion
		case SC_ASPDPOTION1: //Awakening Potion
		case SC_ASPDPOTION2: //Berserk Potion
		case SC_ASPDPOTION3: //Apaan nih?
		...
		case SC_COMA: //Coma. Sends a char to 1HP. If val2, do not zap sp
		{
			int hpchange=0;
			if( map_flag_gvg( bl->m )) { 
				hpchange=(status->hp)/4; 
			} else {
				hpchange=status->hp-1; 
			}
			status_zap(bl, hpchange, val2?0:status->sp);
			return 1;
			break;
		}
		...
		case SC_TEARGAS:
			...
			break;
		case SC_SWINGDANCE:
			val2 = 5 * val1; // ASPD and Walk increase: [(Skill Level x 5) + (Voice Lessons Skill Level)] %
			val2 += val3;
			break;
		case SC_SYMPHONYOFLOVER:
			val2 = 12 * val1; // MDEF increase: [(Skill Level x 12) + (Voice Lesson Skill Level) + (Caster's Job Level / 4)] %
			val2 += val3 + val4 / 4;
			break;
		case SC_RUSHWINDMILL:
			val2 = 6 * val1; // ATK increase: [ (Skill Level x 6 ) + ( Voice Lesson Skill Level ) + (Caster's Job Level / 5 ) ] 
			val2 += val3 + val4 / 5;
			break;
		case SC_ECHOSONG:
			val2 = 6 * val1; // DEF increase: [(Skill Level x 6) + (Voice Lesson Skill Level) + (Caster's Job Level / 4)] %
			val2 += val3 + val4 /4;
			break;			
		case SC_MOONLITSERENADE:
			val2 = 6 * val1; // MATK increase: [(Skill Level x 6) + (Voice Lesson Skill Level) + (Caster's Job Level / 5)]
			val2 += val3 + val4 / 5;
			break;
		case SC_HARMONIZE:
			val2 = 5 + 5 * val1; // Now reduces all stat by - (5 + 5 * Skill Level)
			break;
		case SC_VOICEOFSIREN:
			val4 = tick / 2000;
			tick = 2000;
		...
		case SC_GLOOMYDAY:
			...
			break;
		case SC_GLOOMYDAY_SK:
			val2 = (sd ? pc_checkskill(sd, WM_LESSON) : 5) * 5 + val1 * 10;
			if (sd)
				if( pc_isriding(sd,OPTION_RIDING|OPTION_RIDING_DRAGON) ) pc_setriding(sd, 0); 
			break;
		...
		case SC_FORCEOFVANGUARD:
			...
			break;
		case SC_EXEEDBREAK:
			val1 *= 100; // ATK = [{(Skill Level x 100) + (Caster's Job Level x 10)} + (Weapon Weight x Weapon Level) x (Caster’s Base Level / 100)] %
			if( sd )
			{	// Chars.
				...
				val1 += 10 * sd->status.job_level; // 15 * job_lvl
			}
			else	// Mobs
		...
		case SC_PRESTIGE: // Magic avoidance: [(Caster's INT + LUK) x (Skill Level / 20) x (Caster's Base Level / 200)] + (Skill Level) %
			val2 = (status->int_+status->luk) * (val1 /20) * (status_get_lv(bl) /200) + val1;	// Chance to evade magic damage.
			val1 *= 15; // DEF increase: [{(Skill Level x 15) + (Defending Aura Skill Level x 10)} x Caster’s Base Level / 100]
			if( sd )
		...
		case SC_INSPIRATION:
			if( sd )
			{
				val2 = (status_get_lv(bl) / 10) + (sd->status.job_level / 5);
				val3 = sd->status.job_level;
			}
			val4 = tick / 1000;
			tick = 1000;
			status_change_clear_buffs(bl,3); //Remove buffs/debuffs
		...
		case SC_CRESCENTELBOW:
			val2 = 50 + 5 * val1 + (sd ? sd->status.job_level : 50) / 2;
			val_flag |= 1|2;
			break;
		case SC_LIGHTNINGWALK:
			val1 = 40 + 5 * val1 + (sd ? sd->status.job_level : 50) / 2;
			val_flag |= 1;
			break;
		case SC_RAISINGDRAGON:
			...
			break;
		case SC_GT_CHANGE:
			val2 = (status->agi * val1) / 60;
			break;
		case SC_GT_REVITALIZE:
			val2 = 3 * val1;
			val3 = 50 + 30 * val1; //HP recovery
			break;
		...
		case SC_ANGRIFFS_MODUS:
			val2 = 70 + 30*val1;
			val3 = 50 + 20*val1; // Flee Bonus.
			val4 = 60 + 20*val1; // Def Bonus
			break;
		case SC_GOLDENE_FERSE:
			val2 = 20 + 10*val1; // Flee Bonus.
			val3 = 10 + 4*val1; // Aspd Bonus
...
#define sc_timer_next(t,f,i,d) \
	...
	case SC_ELECTRICSHOCKER:
		if( --(sce->val4) >= 0 )
		{
			status_charge(bl, 0, status->max_sp / 100 * (5 *sce->val1) );
			sc_timer_next(1000 + tick, status_change_timer, bl->id, data);
			return 0;
		}
		break;
	...
	case SC__INVISIBILITY:
		if( --(sce->val4) >= 0 )
		{
			if( !status_charge(bl, 0, (status->sp * (12 - sce->val1*2)) / 100) )// 12% - 2 * skilllv.
				break;
			sc_timer_next(1000 + tick, status_change_timer, bl->id, data);
			return 0;
		}
	...
	case SC_TEARGAS:
		...
		{
			bool flag = 0;
			int damage = status->max_hp * 5 / 100;
			if( status->hp <= damage )
				damage = status->hp - 1; // Cannot Kill

			if( damage > 0 )
			{ // 5% Damage each 2 seconds
				map_freeblock_lock();
				status_zap(bl,damage,0);
				flag = !sc->data[type]; // Killed? Should not
				map_freeblock_unlock();
...
/*==========================================
 * DB reading.
 * job_base.txt   - weight, hp, sp
 * job_aspd.txt   - aspd table
 * job_stat.txt   - job level stat bonuses
 * job_third.txt  - third hp/sp bonus
 * size_fix.txt   - size adjustment table for weapons
 * refine_db.txt  - refining data table
 *------------------------------------------*/
static bool status_readdb_job_base(char* fields[], int columns, int current)
{// Job-specific values (weight, HP, SP)
	int idx, class_;
	...
	if(!pcdb_checkid(class_))
	{
		ShowWarning("status_readdb_job_base: Invalid job class %d specified.\n", class_);
		return false;
	}
...
static bool status_readdb_job_aspd(char* fields[], int columns, int current)
{// Job-specific values (ASPD)
	int idx, class_;
	unsigned int i;
	...
	if(!pcdb_checkid(class_))
	{
		ShowWarning("status_readdb_job_aspd: Invalid job class %d specified.\n", class_);
		return false;
	}
	idx = pc_class2idx(class_);

	for(i = 0; i < MAX_WEAPON_TYPE; i++)
		aspd_base[idx][i] = atoi(fields[i+1]);
	for(i = 0; i < 4; i++) //Shield, Dagger, Sword, Axe
		aspd_penalties[idx][i] = atoi(fields[i+1+MAX_WEAPON_TYPE]);

	return true;
}

static bool status_readdb_job_stat(char* fields[], int columns, int current)
{
	int idx, class_, i;
	...
	if(!pcdb_checkid(class_))
	{
		ShowWarning("status_readdb_job_stat: Invalid job class %d specified.\n", class_);
		return false;
	}
	idx = pc_class2idx(class_);
...
static bool status_readdb_job_third(char* fields[], int columns, int current)
{
	int idx=0, type, jobs[CLASS_COUNT], job_count;
	unsigned int i, j;

	for (i=0; i<CLASS_COUNT; i++) {
		if (!fields[0]) break;
		jobs[i] = atoi(fields[0]);
		fields[0] = strchr(fields[0],':');
		if (fields[0])
			*fields[0]++=0;
	}
	...
	{
		if(!pcdb_checkid(jobs[i]))
		{
			ShowWarning("status_readdb_job_third: Invalid job class %d specified.\n", jobs[i]);
			continue;
		}
		idx = pc_class2idx(jobs[i]);
		for(j = 2; j < columns && type; j++)
		{
			if (type == 1)
				thirdhp[idx][j-2] = atoi(fields[j]);
			if (type == 2)
				thirdsp[idx][j-2] = atoi(fields[j]);
		}
	}
...
int status_readdb(void)
{
	...
	memset(aspd_penalties, 0, sizeof(aspd_penalties));

	// job_third.txt
	memset(thirdhp, 0, sizeof(thirdhp));
	memset(thirdsp, 0, sizeof(thirdsp));
	...
	// read databases
	//
	sv_readdb(db_path, "job_base.txt",   ',', 5,                 5,                 -1,                            &status_readdb_job_base);
	sv_readdb(db_path, "job_aspd.txt",   ',', 5+MAX_WEAPON_TYPE, 5+MAX_WEAPON_TYPE, -1,                            &status_readdb_job_aspd);
	sv_readdb(db_path, "job_stat.txt",   ',', 1,                 1+MAX_LEVEL,       -1,                            &status_readdb_job_stat);
	sv_readdb(db_path, "job_third.txt",  ',', 2,                 1+MAX_LEVEL,       -1,                            &status_readdb_job_third);
	sv_readdb(db_path, "size_fix.txt",   ',', MAX_WEAPON_TYPE,   MAX_WEAPON_TYPE,    ARRAYLENGTH(atkmods),         &status_readdb_sizefix);
	sv_readdb(db_path, "refine_db.txt",  ',', 3+MAX_REFINE,      3+MAX_REFINE,       ARRAYLENGTH(percentrefinery), &status_readdb_refine);
