...
typedef enum sc_type {
	...
	// ระบบเข้าชมกิลวอร์ by Mr.Postman
	SC_AUDIENCE = 999,
...
struct weapon_atk {
	...
	unsigned short matk; //Refine MATK
...
struct status_data {
	...
	short 
		hit, flee, cri, flee2,
		def2, mdef2, mdef,
		aspd_rate, aspd_rate2, def, baspd;
	unsigned char
		def_ele, ele_lv,
		size, race;
...
#define status_get_wmatk2(bl) status_get_status_data(bl)->rhw.matk
...
#define status_get_status_matk(bl) status_get_status_data(bl)->smatk
