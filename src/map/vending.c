...
void vending_vendinglistreq(struct map_session_data* sd, int id)
{
	...
	if( pc_readglobalreg( vsd, "VENDING_MODE" ) == 2 )
	{
		char buf[255];
		sprintf( buf, "-ร้านค้าของ %s ได้ใช้ %s เป็นเงื่อนไขในการซื้อสินค้า-", vsd->status.name, itemdb_exists( pc_readglobalreg( vsd, "VENDING_ITEM_REQ" ) )->jname );
		clif_broadcast( &sd->bl, buf, strlen( buf ) + 1, 0x10, SELF);
	}
...
void vending_purchasereq(struct map_session_data* sd, int aid, int uid, const uint8* data, int count)
{
	...
	v_mode = pc_readglobalreg( vsd, "VENDING_MODE" );
	if( v_mode == 2 )
		item_req = pc_readglobalreg( vsd, "VENDING_ITEM_REQ" );
	...
	for( i = 0; i < count; i++ )
	{
		...
		if( v_mode == 2 )
			itmx = pc_search_inventory( sd, item_req );

		z += ((double)vsd->vending[j].value * (double)amount);
		if(  v_mode != 2 && ( z > (double)sd->status.zeny || z < 0. || z > (double)MAX_ZENY ) )
		{
			clif_buyvending(sd, idx, amount, 1); // you don't have enough zeny
			return;
		}

		if( v_mode == 2 && ( itmx < 0 || z > sd->status.inventory[itmx].amount || z < 0 || z > MAX_AMOUNT) )
		{
			char buf[255];
			sprintf( buf, "คุณต้องการ %s จำนวน %d ea. ในการซื้อสินค้าจากร้านค้านี้", itemdb_exists( item_req )->jname, ( int )z );
			clif_broadcast( &sd->bl, buf, strlen( buf ) + 1, 0x10, SELF);
			return;
		}

		if( v_mode == 2 )
			itmx = pc_search_inventory( vsd, item_req );

		if( ( v_mode != 2 && ( z + (double)vsd->status.zeny > (double)MAX_ZENY && !battle_config.vending_over_max ) ) ||
			( v_mode == 2 && ( z + vsd->status.inventory[itmx].amount, z > MAX_AMOUNT ) ) )
		{
		...
		if( v_mode == 2 )
		{
			wv += itemdb_weight( item_req ) * amount;
			if( wv + vsd->weight > vsd->max_weight )
			{
				char buf[255];
				strcpy( buf, "ไม่สามารถซื้อสินค้าได้ เนื่องจากเจ้าของร้านน้ำหนักเกิน" );
				clif_broadcast( &sd->bl, buf, strlen( buf ) + 1, 0x10, SELF);
	...
	if( v_mode == 2 )
	{
		...
		item_tmp.nameid = item_req;
		item_tmp.identify = 1;

		itmx = pc_search_inventory( sd, item_req );
		pc_delitem( sd, itmx, ( int )z, 0, 0, LOG_TYPE_VENDING );
		pc_additem( vsd, &item_tmp, ( int )z , LOG_TYPE_VENDING);
	} else {
		pc_payzeny(sd, (int)z);
...
//[Ind] for overlapping checks.
static int ind_meaningless(struct block_list *bl,va_list ap)
{
	struct map_session_data *sd = (struct map_session_data *)bl;
	int id=0;
	id=va_arg(ap,int);
	if(id!=0) { // not calling from N.P.C. thingy
		if( id != bl->id && ( sd && sd->vender_id ) ) // we check for vender id so we won't get this warning if, for example, a player is passing by your loc while you are building your vending.
			return 1;
	} else //its being called from npc foreach, therefore we just need to return 1.
		return 1;

	return 0;
}
int ind_check_areanpc(int m, int x, int y, int range)
{
	int x0,y0,x1,y1,r=0,id=0;
	x0 = max(x-range, 0);
	y0 = max(y-range, 0);
	x1 = min(x+range, map[m].xs-1);
	y1 = min(y+range, map[m].ys-1);
	r=map_foreachinarea(ind_meaningless,m,x0,y0,x1,y1,BL_NPC,id);
	return (r);
}
int ind_check_areapc(int id, int m, int x, int y, int range)
{
	int x0,y0,x1,y1,r=0;
	x0 = max(x-range, 0);
	y0 = max(y-range, 0);
	x1 = min(x+range, map[m].xs-1);
	y1 = min(y+range, map[m].ys-1);
	r=map_foreachinarea(ind_meaningless,m,x0,y0,x1,y1,BL_PC,id);
	return (r);
}
