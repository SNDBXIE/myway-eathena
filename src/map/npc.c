...
static const char* npc_parse_script(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	for (i = 0; i < nd->u.scr.label_list_num; i++)
	{
		...
		{
			struct event_data* ev;
			char buf[EVENT_NAME_LENGTH]; // 24 for npc name + 24 for label + 2 for a "::" and 1 for EOS
			snprintf(buf, ARRAYLENGTH(buf), "%s::%s", nd->exname, lname);
...
int npc_duplicatetombstone( int m, int x, int y, int mvpid, int killerid)
{
	...
	if( npc_name2id(newname) != NULL )
	{ // Name of NPC in use.
		ShowError("npc_duplicatetombstone: o Name of NPC (%s) already in use for o MvP %d no map %s.\n", newname, mvpid, map[m].name);
		return 1;
	}

		snprintf(w1, sizeof(w1), "%s,%d,%d,4", map[m].name, x, y);
		snprintf(w2, sizeof(w2), "duplicate(TOMBSTONE)");
		snprintf(w3, sizeof(w3), "Tomb#%d::%s", mvpid, newname);
