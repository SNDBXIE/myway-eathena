...
#define NC_SKILL	0x01
#define NC_ATTACK	0x02
#define NC_GEMSTONE	0x04
#define NC_SKILLSP	0x08
#define NC_ALLFLAG	0x0F
#define MAX_RUNE 20
#define MAX_RAGE 15
...
struct map_session_data {
	...
	struct {
		...
		short rate;
		unsigned short sp;
	} sp_skill[MAX_PC_BONUS];

	struct {
		int id;
		short move;
	} move_skill[MAX_PC_BONUS];
	...
	struct f_dodge full_dodge[3];
	

	struct { //skillatk raises bonus dmg% of skills, skillheal increases heal%, skillblown increases bonus blewcount for some skills.
		unsigned short id;
		short val;
		short casttype; //cast type 0=castrate 1=fixed castrate 2=variable castrate 3=variable cast 4=fixed cast
	...
	struct {
		int id;
		int val;
	} cooldown[MAX_PC_BONUS],cooldownrate[MAX_PC_BONUS],spcost[MAX_PC_BONUS],spcostrate[MAX_PC_BONUS];
	...
	unsigned short add_drop_rate;
	// Epoque's Expansion Pack
	int add_matk;
	int magic_damage_return2;
	unsigned short heal_attack;

	// zeroed vars end here.

	int castrate, delayrate, hprate, sprate, dsprate;
	int hprecov_rate, sprecov_rate;
	int wmatk, matk_rate, wmatk_rate, ematk, eatk;
	int critical_rate, hit_rate, flee_rate, flee2_rate, def_rate, def2_rate, mdef_rate, mdef2_rate;
	
	int fixed_castrate, fixed_cast , variable_castrate, variable_cast;
	...
	bool pvp;	//PVP Area by Mr.Postman
...
static inline bool pcdb_checkid(unsigned short class_)
{
	if (class_ < JOB_MAX_BASIC ||
