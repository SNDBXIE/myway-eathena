...
{
	struct map_session_data* sd = map_id2sd(sid);
	struct map_session_data* dstsd = map_id2sd(tid);

	if( sd == NULL || dstsd == NULL )
		return 0;

	if( ( sd->pvp && dstsd->pvp ) == 1 )
		return 1;
...
int do_init(int argc, char *argv[])
{
	...
	duel_list[MAIN_DUEL_ROOM].max_players_limit = MAX_DUEL; // ﾁﾒ｡ﾊﾘｴ 1024 ､ｹ
