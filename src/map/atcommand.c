...
ACMD_FUNC(rates)
{
	...
	{
		snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,8888), // Drop Zeny Rates: Normal %.2fx / Boss %.2fx
...
ACMD_FUNC(leaveguild)
{
	...
	{
		clif_displaymessage(fd, msg_txt(sd,800)); // You need to be a member of a guild to use this command.
		return -1;
	}
	
	if (!strcmp(g->master, sd->status.name))
	{
		clif_displaymessage(fd, msg_txt(sd,802)); // Guild masters can't directly leave the guild. Use [@breakguild] instead.
		return -1;
	}
	...
	if (message[0])
		...
		guild_leave(sd, sd->status.guild_id, sd->status.account_id, sd->status.char_id, msg_txt(sd,801));
...
ACMD_FUNC(guildinvite)
{
	...
	{
		clif_displaymessage(fd, msg_txt(sd,807)); // You do not have enough authority to invite players to the guild.
	...
	if(message[0])
	{
		...
		if(target_sd != NULL)
		{
			if(target_sd->status.guild_id!=0)
				clif_displaymessage(fd, msg_txt(sd,805)); // Target player already have a guild.
			else
				guild_invite(sd, target_sd);
		}
		else
		{
			clif_displaymessage(fd, msg_txt(sd,804));
			return -1;
		}
	}
	else
