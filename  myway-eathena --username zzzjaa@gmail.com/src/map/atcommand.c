...
ACMD_FUNC(mapmove)
{
	...
	if (sd->bl.m >= 0 && map[sd->bl.m].flag.nowarp && battle_config.any_warp_GM_min_level > pc_isGM(sd) || map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP )) {
...
ACMD_FUNC(jobchange)
{
	...
	if (!message || !*message || sscanf(message, "%d %d", &job, &upper) < 1)
	{
		...
		if (!found) {
			...
			clif_displaymessage(fd, "4190 Extended Super Novice  4191 Extended Super Baby");
...
/*==========================================
 * @reloadstatusdb - reloads job_base.txt job_aspd.txt job_stat.txt job_third.txt refine_db.txt size_fix.txt
 *------------------------------------------*/
...
/*==========================================
 * Trigger any NPC label by Mercurial
 *------------------------------------------*/
ACMD_FUNC(trigger)
{
	 if( !message || !*message) // checks if the message contains something 0_0
		 return -1;
	npc_event_doall(message);
	sprintf(atcmd_output,"You have triggered %s label.",message);
	return 0;
}
...
int atcommand_ind_gw(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (sd->status.guild_id == 0 || (g = guild_search(sd->status.guild_id)) == NULL )//|| strcmp(g->master,sd->status.name))
	{
		clif_displaymessage(fd, "You need to be in a guild to use this command.");
		return -1;
	}
	pos = guild_getposition(g,sd);
	
	if( pos != 0 && !(g->position[pos].mode&0x40)) {//only GM or allowed peps are able to use it.
		clif_displaymessage(fd, "You need the guild master's permission to use this command.");
		return -1;
	}
	if( sd->indtick && DIFF_TICK(gettick(), sd->indtick) < 1500 ) {
		clif_displaymessage(fd,"[ GW ] Don't flood it!! you'll be able to use it again within 1 second...");
		return -1;
	}
	if (!message || !*message) {
		char outg[254];
		clif_displaymessage(fd, "Please, enter a message (usage: @guildmes/@gw <message>).");
		if( pos != 0 ) return -1;//non members shouldn't see the stuff below.
		clif_displaymessage(fd, "To edit your permissions the usage: @guildmes/@gw <pos num> <1|0> where 1 allows and 0 disallows.");
		//display the permission list
		clif_displaymessage(fd,"======= Permission List =======");
		sprintf(outg,"[ %s (Master) ] : Allowed (Can't change).",g->position[0].name);
		clif_displaymessage(fd,outg);
		for(i=1;i<MAX_GUILDPOSITION;i++){
			sprintf(outg,"[ %s->%d ] : %s.",g->position[i].name,i,g->position[i].mode&0x40?"Allowed":"Disallowed");
			clif_displaymessage(fd,outg);
		}
		...
		clif_displaymessage(fd,"The position provided does not exist or either cannot be changed (e.g. guild master rights).");
		return -1;
	}
		if( perm < 0 || perm > 1 ) {
			clif_displaymessage(fd,"The permission must be either 1 or 0.");
			return -1;
		}
		if( perm )
			allow = true;
		...
		clif_displaymessage(fd,"Position rights successfully changed, to see the complete permission list type '@gstoragelock'.");
		return 0;
	}
	//#FFAA00
...
ACMD_FUNC(seehp)
{
	...
	if (!sd->state.seehp) {
		sd->state.seehp = 1;
		clif_displaymessage(sd->fd, "You can now see your members' HP.");
	} else {
		sd->state.seehp = 0;
		clif_displaymessage(sd->fd, "Returned to normal state.");
	}
...
void atcommand_basecommands(void) {
	...
	AtCommandInfo atcommand_base[] = {
		...
		{ "trigger",           90,90,     atcommand_trigger },
		{ "whosell",            1,1,      atcommand_whosell },
		{ "gw",                 1,99,    atcommand_ind_gw },
		{ "guildmes",           1,99,    atcommand_ind_gw },
